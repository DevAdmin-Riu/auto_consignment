...
// 협력사별 Chrome Preferences (chrome-launcher prefs → Default/Preferences 파일에 기록됨)
const passwordDisablePrefs = {
...
const vendorPrefs = {
  napkin: passwordDisablePrefs,
  swadpia: passwordDisablePrefs,
  adpia: passwordDisablePrefs,
};
...
function getBrowserArgs(vendorKey) {
  ...
  const vendorArgs = {
    ...
    swadpia: [
      ...
      "--disable-features=PrivateNetworkAccessPermissionPrompt,PrivateNetworkAccessForNavigations,PrivateNetworkAccessForWorkers,PrivateNetworkAccessNonSecureContextsAllowed,LocalNetworkAccessCheck,LocalNetworkAccessChecks,DialMediaRouteProvider",
    ],
    napkin: [
      "--disable-popup-blocking",
      "--disable-notifications",
    ...
    naver: [
      "--disable-popup-blocking", // 주소 검색 팝업
      "--disable-notifications",
      "--disable-web-security", // iframe 크로스 도메인
...
async function getBrowser(vendorKey = null) {
  // 협력사가 바뀌면 브라우저 재시작 (args가 다르기 때문)
  if (
    browserInstance &&
    vendorKey &&
    currentVendorKey &&
    ...
    console.log(
      `[브라우저] 협력사 변경: ${currentVendorKey} → ${vendorKey} (브라우저 재시작)`,
    );
    await resetBrowser();
  }
  ...
  if (!browserInstance) {
    ...
    const prefs = vendorPrefs[vendorKey] || {};
    const { browser, page } = await connect({
      headless: false,
      args,
      customConfig: Object.keys(prefs).length > 0 ? { prefs } : {},
      connectOption: {
        defaultViewport: null, // --start-maximized와 연동 (창 크기에 맞게 자동 조정)
        protocolTimeout: 300000, // 5분 (기본 180초 → 300초, 대용량 파일 업로드 대응)
      },
    });
    ...
    // Chrome 창 OS 포커스 (Interception 키 입력을 위해)
    try {
      const { focusChromeWindow } = require("./shinhan-payment");
      focusChromeWindow();
    } catch (e) {}
  }
