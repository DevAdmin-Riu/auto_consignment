...
function getBrowserArgs(vendorKey) {
  ...
  const baseArgs =
    process.env.NODE_ENV === "production"
      ? ["--disable-blink-features=AutomationControlled"] // 프로덕션: 샌드박스 유지
      : [
          "--no-sandbox",
...
/**
 * 안전한 페이지 이동 (네트워크 실패 시 재시도)
 * @param {object} page - Puppeteer page 인스턴스
 * @param {string} url - 이동할 URL
 * @param {object} options - { waitUntil, timeout, retries }
 */
async function safeGoto(page, url, options = {}) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const isNetworkError = e.message.includes("net::ERR_") ||
        e.message.includes("Navigation timeout") ||
        e.message.includes("timeout") ||
        e.message.includes("ECONNREFUSED");

      if (attempt < retries && isNetworkError) {
        console.log(`[safeGoto] 페이지 로드 실패 (${attempt}/${retries}): ${e.message.substring(0, 80)}`);
        console.log(`[safeGoto] ${3 * attempt}초 후 재시도...`);
