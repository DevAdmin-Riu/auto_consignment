 ...
 */
async function processShinhanCardPayment(paymentFrame, focusPage, paymentMethod = "phone") {
  try {
    // Step 1: "다른결제" 탭 클릭
    ...
    if (paymentMethod === "card") {
      // ========== 카드번호 결제 ==========
      return await processCardNumberPayment(paymentFrame, focusPage);
    } else {
      // ========== 휴대폰번호 결제 (기본) ==========
      return await processPhonePayment(paymentFrame, focusPage);
    }
  } catch (error) {
...
async function processPhonePayment(paymentFrame, focusPage) {
  ...
  if (!typeWithInterception(phone)) {
    return { success: false, error: "휴대폰번호 입력 실패" };
  }
  console.log("[Shinhan] ✅ 휴대폰번호 입력 완료");
  await delay(500);
  ...
  for (let retry = 0; retry < 10; retry++) {
    birthInputFound = await paymentFrame.evaluate(() => {
      ...
      for (const input of inputs) {
        ...
        if (
          placeholder.includes("YYMMDD") ||
          placeholder.includes("생년월일") ||
          id.includes("birth") ||
          name.includes("birth")
        ) {
          input.focus();
  ...
  if (!typeWithInterception(birth)) {
    return { success: false, error: "생년월일 입력 실패" };
  }
