...
function focusChromeWindow() {
  try {
    execSync(`powershell -Command "Add-Type -TypeDefinition 'using System; using System.Runtime.InteropServices; public class Win { [DllImport(\\\"user32.dll\\\")] public static extern bool SetForegroundWindow(IntPtr hWnd); [DllImport(\\\"user32.dll\\\")] public static extern bool ShowWindow(IntPtr hWnd, int nCmdShow); }'; $p = Get-Process chrome -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowHandle -ne 0 } | Select-Object -First 1; if ($p) { [Win]::ShowWindow($p.MainWindowHandle, 9); [Win]::SetForegroundWindow($p.MainWindowHandle) }"`, { timeout: 5000 });
    console.log("[Shinhan] Chrome 창 포커스 완료 (OS 레벨)");
  } catch (e) {
