...
const SCRIPTS_DIR = path.join(__dirname, "../scripts/interception");
const TEMP_DIR = path.join(__dirname, "../temp");
const TYPE_SCRIPT = path.join(SCRIPTS_DIR, "type_keyboard_only.py");
const KB_DEVICE_CACHE = path.join(TEMP_DIR, "kb_device.txt");

const delay = (ms) => new Promise((r) => setTimeout(r, ms));

/**
 * Chrome 창을 OS 레벨에서 강제 포그라운드로 올리기
 * Interception 키 입력은 포그라운드 창에만 전달되므로 필수
 */
function focusChromeWindow() {
  try {
    // SW_SHOWMAXIMIZED(3)으로 창 최대화 유지 (SW_RESTORE(9)는 normal 크기로 축소시켜 반응형 UI 변화 유발)
    execSync(`powershell -Command "Add-Type -TypeDefinition 'using System; using System.Runtime.InteropServices; public class Win { [DllImport(\\\"user32.dll\\\")] public static extern bool SetForegroundWindow(IntPtr hWnd); [DllImport(\\\"user32.dll\\\")] public static extern bool ShowWindow(IntPtr hWnd, int nCmdShow); [DllImport(\\\"user32.dll\\\")] public static extern bool GetWindowRect(IntPtr hWnd, out RECT lpRect); [DllImport(\\\"user32.dll\\\")] public static extern void mouse_event(uint dwFlags, int dx, int dy, uint dwData, IntPtr dwExtraInfo); [DllImport(\\\"user32.dll\\\")] public static extern bool SetCursorPos(int X, int Y); [StructLayout(LayoutKind.Sequential)] public struct RECT { public int Left; public int Top; public int Right; public int Bottom; } }'; $p = Get-Process chrome -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowHandle -ne 0 } | Select-Object -First 1; if ($p) { [Win]::ShowWindow($p.MainWindowHandle, 3); [Win]::SetForegroundWindow($p.MainWindowHandle); $r = New-Object Win+RECT; [Win]::GetWindowRect($p.MainWindowHandle, [ref]$r); $x = [int](($r.Left + $r.Right) / 2); $y = $r.Top + 15; [Win]::SetCursorPos($x, $y); [Win]::mouse_event(0x0002, 0, 0, 0, [IntPtr]::Zero); [Win]::mouse_event(0x0004, 0, 0, 0, [IntPtr]::Zero) }"`, { timeout: 5000 });
    console.log("[Shinhan] Chrome 창 포커스 + 클릭 완료 (OS 레벨)");
  } catch (e) {
    console.log(`[Shinhan] Chrome 창 포커스 실패 (무시): ${e.message?.substring(0, 50)}`);
  }
}
...
async function processPhonePayment(paymentFrame, focusPage) {
  ...
  // 휴대폰번호 타이핑 — bringToFront 후 직접 클릭으로 포커스 확보
  await focusPage.bringToFront();
  await delay(300);

  // 방법 A: puppeteer .click()으로 직접 클릭 (현재 사용)
  const phoneInput = await paymentFrame.$('input[placeholder*="없이"], input[placeholder*="휴대폰"], input[type="tel"]');
  if (phoneInput) {
    await phoneInput.click();
    await delay(300);
    console.log("[Shinhan] 휴대폰번호 input 클릭 완료");
  } else {
    // 폴백: evaluate로 포커스
    await paymentFrame.evaluate(() => {
      ...
      const telInput = document.querySelector('input[type="tel"]');
      if (telInput) { telInput.focus(); telInput.click(); }
    });
    await delay(300);
    console.log("[Shinhan] 휴대폰번호 input 포커스 (폴백)");
  }

  // 방법 B: 기존 방식 (방법 A가 안 되면 이걸로 교체)
  // await paymentFrame.evaluate(() => {
  //   const inputs = document.querySelectorAll("input");
  //   for (const input of inputs) {
  //     const placeholder = input.placeholder || "";
  ...
  // 휴대폰번호 입력 + 검증 (최대 3회 재시도)
  let phoneVerified = false;
  for (let attempt = 0; attempt < 3; attempt++) {
    await delay(200);
    console.log(`[Shinhan] 휴대폰번호 Interception 입력 중... (시도 ${attempt + 1}/3)`);
    ...
    await paymentFrame.evaluate(() => {
      ...
      for (const input of inputs) {
        const placeholder = input.placeholder || "";
        if (placeholder.includes("없이") || placeholder.includes("휴대폰") || input.type === "tel") {
          input.value = "";
          input.focus();
          input.click();
          return;
    ...
    if (!typeWithInterception(phone)) {
      console.log(`[Shinhan] ⚠️ Interception 입력 실패 (시도 ${attempt + 1}/3)`);
      continue;
    }
    await delay(500);

    // 입력값 검증
    const inputValue = await paymentFrame.evaluate(() => {
      const inputs = document.querySelectorAll("input");
      for (const input of inputs) {
        const placeholder = input.placeholder || "";
        if (placeholder.includes("없이") || placeholder.includes("휴대폰") || input.type === "tel") {
    ...
    if (inputValue.length >= phone.length) {
      console.log(`[Shinhan] ✅ 휴대폰번호 입력 완료 (검증 OK: ${inputValue.length}자)`);
      phoneVerified = true;
      break;
    } else {
      console.log(`[Shinhan] ⚠️ 휴대폰번호 입력 불완전: ${inputValue.length}자 / ${phone.length}자 → 재시도`);
    }
  }
  if (!phoneVerified) {
    return { success: false, error: "휴대폰번호 입력 실패 (3회 재시도 후)" };
  }
  await delay(1000);
  ...
  console.log("[Shinhan] ✅ 생년월일 입력 완료");
