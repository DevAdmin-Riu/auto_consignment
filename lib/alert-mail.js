...
const DEV_EMAIL = "xdswwwj@riupack.com";

/**
 * launcher-config.json에서 poEmail(담당자 이메일) 읽기
 */
function getPoEmail() {
  try {
    const configPath = path.join(__dirname, "..", "launcher", "launcher-config.json");
    const config = JSON.parse(fs.readFileSync(configPath, "utf-8"));
    return config.poEmail || DEV_EMAIL;
  } catch (e) {
    return DEV_EMAIL;
  }
}
...
function sendAlertMail({ subject, body, vendor, purchaseOrderId }) {
  try {
    ...
    const poEmail = getPoEmail();
    t.sendMail({
      from: "xdswwwj@gmail.com",
      to: poEmail,
      cc: `${DEV_EMAIL}, kyungjun0928@riupack.com`,
      subject: fullSubject,
      html,
    }).catch((e) => {
 ...
 * 결제금액 파싱 실패 알림
 */
function alertPaymentParsingFailed({ vendor, purchaseOrderId, openMallOrderNumber, paymentAmount, parsingDetail }) {
  const issues = [];
  ...
  // 파싱 상세 정보 HTML
  let detailHtml = "";
  if (parsingDetail && typeof parsingDetail === "object") {
    const entries = Object.entries(parsingDetail)
      .map(([key, val]) => `${key}: ${val ?? "null"}`)
      .join("<br>");
    detailHtml = `<br><b>파싱 상세:</b><br>${entries}`;
  }
