...
async function createPaymentLogs(authToken, payments) {
  ...
  // .env에서 카드 타입 매핑 (shinhan→SHINHAN, bc→BC)
  const cardTypeMap = { shinhan: "SHINHAN", bc: "BC" };
  const envCardType = getEnv("PAYMENT_CARD_TYPE") || "";
  const paymentCard = cardTypeMap[envCardType.toLowerCase()] || null;
  ...
  const input = payments
    ...
    .map((p) => ({
      ...
      paymentCard: p.paymentCard || paymentCard,
    }));
