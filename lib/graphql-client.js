...
async function updateOpenMallOrderNumbers(authToken, products) {
  ...
  for (const p of products) {
    ...
    const orderLineIdList = p.orderLineIds || (p.orderLineId ? [p.orderLineId] : []);
 ...
 * 결제 로그 저장
 * @param {string} authToken
 * @param {Array} payments - [{ vendor, paymentAmount, purchaseOrderId, orderLineId }, ...]
 */
async function createPaymentLogs(authToken, payments) {
  ...
  const input = payments
    ...
    .map((p) => ({
      ...
      purchaseOrder: p.purchaseOrderId || null,
      orderLine: p.orderLineId || null,
    }));
  ...
  console.log(`[GraphQL] 결제 로그 저장: ${input.length}건, 총액: ${input.reduce((sum, p) => sum + p.paymentAmount, 0)}원`);
...
async function processFulfillment(authToken) {
  ...
  // 위탁 출고: 각 OrderLine을 별도 Fulfillment로 처리 (개별 배송 추적 필요)
...
async function saveOrderResults(authToken, data) {
  ...
  // 디버그: 전달받은 데이터 구조 확인
  console.log(`[GraphQL] DEBUG products 구조:`, JSON.stringify(products.map(p => ({
    orderLineIds: p.orderLineIds,
    orderLineId: p.orderLineId,
    openMallOrderNumber: p.openMallOrderNumber,
  }))));
  ...
  // ============================================
  // 주문번호 누락 체크 (openMallOrderNumber가 null인 항목)
  // ============================================
  const missingOrderNumberProducts = products.filter(
    (p) => (p.orderLineIds?.length > 0 || p.orderLineId) && !p.openMallOrderNumber
  );
  ...
  if (missingOrderNumberProducts.length > 0) {
    ...
    // 주문번호 누락 에러 로그 생성
    // step은 소문자 (Saleor AutomationStep enum value), errorCode는 대문자 (AutomationErrorCode enum value)
    const missingOrderNumberErrors = missingOrderNumberProducts.map((p) => ({
      vendor: vendor,
      automationType: "ORDER",
      step: "order_confirmation",  // Saleor enum value는 소문자
      errorCode: "ORDER_NUMBER_MISSING",  // Saleor에 추가 필요
      errorMessage: `주문번호 누락: orderLineIds=${JSON.stringify(p.orderLineIds || p.orderLineId)}`,
      purchaseOrderId,
      purchaseOrderLineId: p.orderLineIds?.[0] || p.orderLineId || null,
      productVariantVendorId: p.productVariantVendorId || null,
    }));

    // 기존 automationErrors에 추가
  ...
  if (success) {
    ...
    if (missingOrderNumberProducts.length > 0) {
      const errorResult = await createAutomationErrors(authToken, automationErrors);
    ...
    // 디버그: 필터링 전 상태 확인
    console.log(`[GraphQL] DEBUG products count: ${products.length}, poLineIds count: ${poLineIds.length}`);
    console.log(`[GraphQL] DEBUG missingOrderNumberProducts count: ${missingOrderNumberProducts.length}`);
    ...
    if (missingOrderNumberProducts.length > 0) {
      if (products.length === poLineIds.length) {
        ...
        products.forEach((p, index) => {
          if ((p.orderLineIds?.length > 0 || p.orderLineId) && !p.openMallOrderNumber) {
            missingProductIndices.push(index);
          }
        });
        poLineIdsToProcess = poLineIds.filter((_, index) => !missingProductIndices.includes(index));
        excludeCount = missingProductIndices.length;
        console.log(`[GraphQL] DEBUG (인덱스 매칭) missingProductIndices:`, JSON.stringify(missingProductIndices));
        ...
        console.log(`[GraphQL] DEBUG (개별처리) 단일 상품 주문번호 누락 - 전체 제외`);
        ...
        console.log(`[GraphQL] DEBUG (기타) validCount: ${validCount}, 앞에서 ${validCount}개만 처리`);
      }
    }

    console.log(`[GraphQL] 대행접수 대상: 전체 ${poLineIds.length}건, 유효 ${poLineIdsToProcess.length}건, 제외 ${excludeCount}건`);
    ...
    if (poLineIdsToProcess.length > 0) {
      ...
      console.log("[GraphQL] ⚠️ 모든 상품의 주문번호가 누락되어 대행접수/출고처리 스킵");
