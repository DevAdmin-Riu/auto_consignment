...
async function callGraphQL(authToken, query, variables = {}) {
  ...
  if (!graphqlUrl) {
    console.error("[GraphQL] GRAPHQL_URL이 설정되지 않았습니다. setConfig() 또는 환경변수를 확인하세요.");
    return null;
  }
...
async function updateOpenMallOrderNumbers(authToken, products) {
  console.log("[GraphQL] 주문번호 업데이트 입력:", JSON.stringify(products, null, 2));
  ...
  if (input.length === 0) {
    console.log("[GraphQL] 업데이트할 주문번호 없음 - orderLineId(s) 또는 openMallOrderNumber가 없음");
    console.log("[GraphQL] products 상세:", products.map(p => ({ orderLineId: p.orderLineId, orderLineIds: p.orderLineIds, openMallOrderNumber: p.openMallOrderNumber })));
    return null;
  }
 ...
 * 가격 불일치 저장
 * @param {string} authToken
 * @param {string} defaultPurchaseOrderId - 기본 발주 ID (fallback용)
 * @param {Array} mismatches - [{ productVariantVendorId, vendorPriceExcludeVat, openMallPrice, purchaseOrderId? }, ...]
 */
async function createPriceMismatches(authToken, defaultPurchaseOrderId, mismatches) {
  const input = mismatches
    .filter((p) => p.productVariantVendorId)
    .map((p) => ({
      productVariantVendorId: p.productVariantVendorId,
      purchaseOrderId: p.purchaseOrderId || defaultPurchaseOrderId, // 개별 상품의 purchaseOrderId 우선
 ...
 * @param {string} authToken
 * @param {string} defaultPurchaseOrderId - 기본 발주 ID (fallback용)
 * @param {Array} mismatches - [{ productVariantVendorId, reason, purchaseOrderId? }, ...]
 */
async function createOptionsMismatches(authToken, defaultPurchaseOrderId, mismatches) {
  const input = mismatches
    .filter((p) => p.productVariantVendorId)
    .map((p) => ({
      productVariantVendorId: p.productVariantVendorId,
...
async function saveOrderResults(authToken, data) {
  ...
  if (success) {
    ...
    summary.orderNumbers = successResults[0].status === "fulfilled" ? "OK" : "FAIL";
    summary.priceMismatch = successResults[1].status === "fulfilled" ? "OK" : "FAIL";

    // 2단계: 대행접수 → 출고처리 (순차)
    if (lineIds.length > 0) {
      try {
        // 대행접수
        const receiveResult = await receivePurchaseOrderLines(authToken, lineIds);
    ...
    summary.optionsMismatch = failResults[0].status === "fulfilled" ? "OK" : "FAIL";
