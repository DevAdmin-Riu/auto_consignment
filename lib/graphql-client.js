...
async function createPriceMismatches(authToken, purchaseOrderId, mismatches) {
  const input = mismatches
    ...
    .filter((p) => {
      // 가격 차이 0원이면 불일치 기록 안 함
      const diff = Math.abs((p.vendorPriceExcludeVat || 0) - (p.openMallPrice || 0));
      if (diff === 0) {
        console.log(`[GraphQL] 가격 차이 0원 → 불일치 제외: ${p.productVariantVendorId}`);
        return false;
      }
      return true;
    })
...
async function saveOrderResults(authToken, data) {
  ...
  if (success) {
    ...
    // priceMismatches에 openMallOrderNumber 매핑 (products에서 가져오기)
    if (priceMismatches.length > 0 && products.length > 0) {
      ...
      for (const pm of priceMismatches) {
        if (!pm.openMallOrderNumber) {
          // 개별 주문: productVariantVendorId로 매칭
