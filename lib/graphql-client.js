...
/**
 * 발주서 발송 (PurchaseOrderSend)
 * DRAFT → PENDING 상태 변경
 * @param {string} authToken
 * @param {string} purchaseOrderId - PurchaseOrder ID
 */
async function sendPurchaseOrder(authToken, purchaseOrderId) {
  if (!purchaseOrderId) {
    console.log("[GraphQL] 발주서 발송 스킵: purchaseOrderId 없음");
    return null;
  }

  console.log(`[GraphQL] 발주서 발송: ${purchaseOrderId}`);

  const query = `
    mutation PurchaseOrderSend($id: ID!) {
      purchaseOrderSend(id: $id) {
        purchaseOrder { id isSent }
        errors: purchaseOrderErrors { code message field }
      }
    }
  `;

  return callGraphQL(authToken, query, { id: purchaseOrderId });
}
...
async function saveOrderResults(authToken, data) {
  ...
  if (success) {
    ...
    // priceMismatches에 openMallOrderNumber 매핑
    // 주의: 각 벤더에서 이미 단가(unit price)로 계산해서 보내므로 qtyPerUnit 곱셈 불필요
    ...
    if (poLineIdsToProcess.length > 0) {
      try {
        // 발주서 발송 (DRAFT → PENDING)
        const sendResult = await sendPurchaseOrder(authToken, purchaseOrderId);
        summary.send = sendResult ? "OK" : "FAIL";

        if (summary.send !== "OK") {
          console.error("[GraphQL] 발주서 발송 실패 → 접수/출고 스킵");
          throw new Error("발주서 발송 실패");
        }
...
/**
 * PurchaseOrderLine n8nInfo 업데이트
 * 기존 n8nInfo를 읽어서 merge 후 업데이트
 *
 * @param {string} authToken
 * @param {string} poLineId - PurchaseOrderLine ID
 * @param {Object} updates - 업데이트할 필드들 (기존 값과 merge)
 */
async function updatePoLineN8nInfo(authToken, poLineId, updates) {
  if (!poLineId) {
    console.log("[GraphQL] n8nInfo 업데이트 스킵: poLineId 없음");
    return null;
  }

  // 1. 기존 n8nInfo + purchaseOrderId 조회
  const queryResult = await callGraphQL(authToken, `
    query GetPoLineN8nInfo($id: ID!) {
  ...
  const existingInfo = queryResult?.data?.purchaseOrderLine?.n8nInfo || {};
  const purchaseOrderId = queryResult?.data?.purchaseOrderLine?.purchaseOrder?.id;
  const merged = { ...existingInfo, ...updates };

  // 2. 업데이트
  const result = await callGraphQL(authToken, `
    mutation UpdatePoLineN8nInfo($id: ID!, $purchaseOrderId: ID!, $input: PurchaseOrderLineUpdateInput!) {
      purchaseOrderLineUpdate(id: $id, purchaseOrderId: $purchaseOrderId, input: $input) {
        purchaseOrderLine {
          id
          n8nInfo
        }
  ...
  `, { id: poLineId, purchaseOrderId, input: { n8nInfo: merged } });

  console.log(`[GraphQL] n8nInfo 업데이트: ${poLineId} →`, JSON.stringify(merged));
  return result;
}

/**
 * 주문 실패 시 n8nInfo 업데이트 (failCount 증가)
 * failCount >= 3 또는 품절/가격초과 → 발송(DRAFT→PENDING)하여 수동 처리로 전환
 *
 * @param {string} authToken
 * @param {string} poLineId
 * @param {string} purchaseOrderId - 발송 처리용
 * @param {Object} failInfo - { lastError, isPriceGapExceeded, soldOut }
 */
async function updatePoLineFailure(authToken, poLineId, purchaseOrderId, failInfo = {}) {
  try {
    // 기존 n8nInfo 조회해서 failCount 증가
    const queryResult = await callGraphQL(authToken, `
      query GetPoLineN8nInfo($id: ID!) {
        purchaseOrderLine(id: $id) {
          id
    ...
    const existingInfo = queryResult?.data?.purchaseOrderLine?.n8nInfo || {};
    const currentFailCount = existingInfo.failCount || 0;

    const updates = {
      failCount: currentFailCount + 1,
      lastError: failInfo.lastError || null,
      lastFailedAt: new Date().toISOString().split("T")[0],
    };

    if (failInfo.isPriceGapExceeded) updates.isPriceGapExceeded = true;
    if (failInfo.soldOut) updates.soldOut = true;

    await updatePoLineN8nInfo(authToken, poLineId, updates);
    console.log(`[GraphQL] 실패 기록: ${poLineId} (failCount: ${updates.failCount})`);

    // failCount >= 3 또는 품절/가격초과 → 발송하여 수동 처리(접수대기)로 전환
    const shouldMoveToPending = updates.failCount >= 3 || failInfo.isPriceGapExceeded || failInfo.soldOut;
    if (shouldMoveToPending && purchaseOrderId) {
      console.log(`[GraphQL] 재시도 한도 초과 → 발송(접수대기로 전환): ${purchaseOrderId}`);
      try {
        await sendPurchaseOrder(authToken, purchaseOrderId);
        console.log(`[GraphQL] ✅ 접수대기 전환 완료: ${purchaseOrderId}`);
      } catch (sendErr) {
        console.error(`[GraphQL] 접수대기 전환 실패 (무시): ${sendErr.message}`);
      }
    }
  } catch (e) {
    console.error(`[GraphQL] n8nInfo 실패 업데이트 에러 (무시): ${e.message}`);
  }
}

/**
 * 주문 성공 시 n8nInfo에 오픈몰 상품 정보 저장
 *
 * @param {string} authToken
 * @param {string} poLineId
 * @param {Object} productInfo - { productUrl, productName, optionName, openMallProductId }
 */
async function updatePoLineSuccess(authToken, poLineId, productInfo = {}) {
  try {
    const updates = {
      failCount: 0,
      lastError: null,
      isPriceGapExceeded: false,
      soldOut: false,
      orderedAt: new Date().toISOString().split("T")[0],
    };

    if (productInfo.productUrl) updates.productUrl = productInfo.productUrl;
    if (productInfo.productName) updates.productName = productInfo.productName;
    if (productInfo.optionName) updates.optionName = productInfo.optionName;

    if (productInfo.openMallProductId) updates.openMallProductId = productInfo.openMallProductId;
    if (productInfo.openMallProductName) updates.openMallProductName = productInfo.openMallProductName;
    if (productInfo.openMallOption) updates.openMallOption = productInfo.openMallOption;

    await updatePoLineN8nInfo(authToken, poLineId, updates);
    console.log(`[GraphQL] 성공 기록: ${poLineId}`);
  } catch (e) {
    console.error(`[GraphQL] n8nInfo 성공 업데이트 에러 (무시): ${e.message}`);
  }
}
