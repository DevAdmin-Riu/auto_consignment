 ...
 * @param {Array} products - [{ orderLineId, openMallOrderNumber }, ...]
 */
async function updateOpenMallOrderNumbers(authToken, products) {
  ...
  const input = products
    .filter((p) => p.orderLineId && p.openMallOrderNumber)
    .map((p) => ({
  ...
  if (input.length === 0) {
    console.log("[GraphQL] 업데이트할 주문번호 없음 - orderLineId 또는 openMallOrderNumber가 없음");
    console.log("[GraphQL] products 상세:", products.map(p => ({ orderLineId: p.orderLineId, openMallOrderNumber: p.openMallOrderNumber })));
    return null;
  }
...
async function createAutomationErrors(authToken, errors) {
  ...
  const query = `
    mutation AutomationErrorLogBulkCreate($input: [AutomationErrorLogInput!]!) {
