...
async function saveOrderResults(authToken, data) {
  ...
  if (success) {
    ...
    // 2단계: 대행접수 → 출고처리 (순차)
    // 주문번호가 없는 상품의 lineIds는 대행접수/출고처리에서 제외
    const missingLineIds = missingOrderNumberProducts.flatMap(
      (p) => p.orderLineIds || (p.orderLineId ? [p.orderLineId] : [])
    );
    const lineIdsToProcess = missingLineIds.length > 0
      ? lineIds.filter((id) => !missingLineIds.includes(id))
      : lineIds;

    console.log(`[GraphQL] 대행접수 대상: 전체 ${lineIds.length}건, 유효 ${lineIdsToProcess.length}건, 제외 ${missingLineIds.length}건`);

    if (lineIdsToProcess.length > 0) {
      try {
        // 대행접수
    ...
    } else if (lineIds.length > 0 && lineIdsToProcess.length === 0) {
