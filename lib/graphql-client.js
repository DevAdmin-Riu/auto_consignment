...
async function updatePoLineN8nInfo(authToken, poLineId, updates) {
  ...
  const result = await callGraphQL(authToken, `
    mutation UpdatePoLineN8nInfo($id: ID!, $input: PurchaseOrderLineUpdateInput!) {
      purchaseOrderLineUpdate(id: $id, input: $input) {
        purchaseOrderLine {
          id
          n8nInfo
        }
  ...
  `, { id: poLineId, input: { n8nInfo: merged } });
 ...
 * @param {string} authToken
 * @param {string} poLineId
 * @param {Object} productInfo - { openMallProductId, openMallProductName, openMallOption }
 */
