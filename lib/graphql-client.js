...
async function updateOpenMallOrderNumbers(authToken, products) {
  ...
  const orderErrors = result?.data?.orderLineBulkUpdateOpenMallOrderNumber?.orderErrors;
  if (orderErrors && orderErrors.length > 0) {
    console.error("[GraphQL] 주문번호 업데이트 orderErrors:", JSON.stringify(orderErrors));
  }
  return result;
}
...
async function createPriceMismatches(authToken, purchaseOrderId, mismatches) {
  ...
  console.log(`[GraphQL] 가격 불일치 저장: ${input.length}건`, JSON.stringify(input));
  ...
  const result = await callGraphQL(authToken, query, { input });
  const productErrors = result?.data?.openMallPriceMismatchBulkCreate?.productErrors;
  if (productErrors && productErrors.length > 0) {
    console.error("[GraphQL] 가격 불일치 저장 productErrors:", JSON.stringify(productErrors));
  } else {
    const createdCount = result?.data?.openMallPriceMismatchBulkCreate?.createdCount;
...
async function createOptionsMismatches(authToken, purchaseOrderId, mismatches) {
  ...
  console.log(`[GraphQL] 옵션 불일치 저장: ${input.length}건`, JSON.stringify(input));
  ...
  const result = await callGraphQL(authToken, query, { input });
  const productErrors = result?.data?.openMallOptionsMismatchBulkCreate?.productErrors;
  if (productErrors && productErrors.length > 0) {
    console.error("[GraphQL] 옵션 불일치 저장 productErrors:", JSON.stringify(productErrors));
  } else {
    const createdCount = result?.data?.openMallOptionsMismatchBulkCreate?.createdCount;
...
function calculateExpectedPaymentAmount(products) {
  return products.reduce((sum, p) => {
    return sum + Math.round((p.costPriceExcludeVat || 0) * 1.1) * (p.quantity || 1);
  }, 0);
}
...
async function createPaymentLogs(authToken, payments) {
  ...
  const input = payments
    ...
    .map((p) => ({
      ...
      purchaseOrderId: p.purchaseOrderId || null,
      orderLineId: p.orderLineId || null,
      note: p.note || null,
    }));
...
async function saveOrderResults(authToken, data) {
  ...
  const missingOrderNumberProducts = products.filter(
    (p) =>
      (p.orderLineIds?.length > 0 || p.orderLineId) && !p.openMallOrderNumber
  );
  ...
  if (missingOrderNumberProducts.length > 0) {
    ...
    const missingOrderNumberErrors = missingOrderNumberProducts.map((p) => ({
      ...
      errorMessage: `주문번호 누락: orderLineIds=${JSON.stringify(
        (p.orderLineIds || [p.orderLineId]).filter(Boolean).map((id) => {
          try { return Buffer.from(id, "base64").toString("utf-8"); } catch { return id; }
        })
      )}`,
      purchaseOrderId,
