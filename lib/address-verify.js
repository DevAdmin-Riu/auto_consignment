...
async function verifyShippingAddressOnPage(page, shippingAddress, vendor = "") {
  ...
  try {
    ...
    // 4. 상세주소 검증 (입력된 값 기준 — 잘림/건물명 중복 대응)
    if (ourDetail) {
      const normalizedFull = normalizeAddress(displayedAddress.fullText);
      const normalizedDetail = normalizeAddress(ourDetail);
      // 화면이 우리 상세주소를 포함하거나, 우리가 화면을 포함하거나, 앞 10자가 일치하면 통과
      const shortDetail = normalizedDetail.substring(0, 10);
      if (normalizedFull.includes(normalizedDetail) || normalizedDetail.includes(normalizedFull) || normalizedFull.includes(shortDetail)) {
        console.log(`${prefix} 상세주소 매칭 성공: "${ourDetail}"`);
      } else {
        console.error(`${prefix} 상세주소 불일치! 우리: "${ourDetail}"`);
